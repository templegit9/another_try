...
async function initApp() {
    ...
    if (sessionId) {
        // Verify the session ID exists in DB (or create if missing/valid)
        // For simplicity, we just assume it's valid if it's there, but let's try to fetch user
        try {
            let user = await getUserBySessionId(sessionId)
            if (!user) {
                // Determine if we should auto-create or ask to login again.
                // For "Session ID" model, maybe we just auto-create if it's missing but was in local storage?
                // Or maybe clear storage. Let's clear storage to be safe if user not found.
                console.warn('Session ID found in storage but not in DB', sessionId)
...
async function handleLogin(e) {
    ...
    if (!sessionId) {
        document.getElementById('login-error').textContent = 'Please enter a Session ID'
    ...
    try {
        ...
        submitButton.innerHTML = '<span class="material-icons animate-spin">refresh</span> Accessing...'
        ...
        if (!user) {
            ...
            user = await createUserWithSessionId(sessionId)
        }

        // Login successful
        // Store session ID
        localStorage.setItem('custom_session_id', user.id)
        ...
        const errorMessage = error.message || 'Failed to access workspace. Please try again.'
        document.getElementById('login-error').textContent = errorMessage
        document.getElementById('login-error').classList.remove('hidden')
    } finally {
        // Reset button state
        const submitButton = e.target.querySelector('button[type="submit"]')
        if (submitButton) { // Button might be gone if we swapped views
            submitButton.disabled = false
            submitButton.innerHTML = '<span class="material-icons mr-1">login</span> Access Workspace'
        }
    }
}
...
function showAuthScreen() {
    ...
    if (loginForm) {
        // Remove existing listener to avoid duplicates if called multiple times (though {once: true} or cleanup is better)
        // For simple app structure, just ensuring we don't duplicate logic. 
        // We'll replace the element to clear listeners or just assume init only happens once/reload.
